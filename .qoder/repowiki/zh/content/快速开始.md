...
MySpeechRecognition是一个基于Web Speech API的实时语音识别应用，支持浏览器原生识别和讯飞语音识别两种模式。该项目采用现代化的前端技术栈，提供了流畅的用户体验和强大的语音识别功能。

**项目特点：**
- 支持浏览器原生Web Speech API和讯飞语音识别
- 实时语音转文字，支持中间结果和最终结果
- 粒子动画背景，科技感十足的界面设计
- 响应式设计，支持移动端和桌面端
- 自动错误检测和后端切换机制
...
// 设置后端类型
speech.setBackend('xfyun'); // 或 'native'

// 获取当前配置
const config = speech.getXfyunConfig();
