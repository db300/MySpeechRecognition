...
- 错误降级机制：原生API到讯飞API的自动切换逻辑。
- 状态变更通知机制与用户提示策略。
- 完整的最佳实践与调试技巧，并通过源码路径指引具体实现位置。

## 项目结构
该项目采用模块化前端架构，核心由三部分组成：
- 语音识别管理器：统一调度原生Web Speech API与讯飞WebSocket API，负责状态机、错误分发与降级切换。
- 讯飞语音客户端：封装WebSocket鉴权、音频采集与实时识别流程。
...
XR["XfyunSpeech<br/>js/xfyun-speech.js"]
end
subgraph "浏览器能力"
NATIVE["Web Speech API<br/>浏览器原生"]
WS["WebSocket<br/>讯飞服务"]
...
## 架构总览
系统通过“语音识别管理器”作为中枢，根据当前后端类型调用对应实现；当原生API出现网络错误时，自动记录失败标记并在必要时切换至讯飞API；讯飞API负责WebSocket鉴权、音频流传输与识别结果回传。
...
participant X as "讯飞API<br/>WebSocket"
U->>A : 点击麦克风/空格键
A->>S : startListening()
alt 后端为原生
S->>N : start()
...
- 原生API错误处理
  ...
  - network：网络错误，记录失败标记与重试计数；超过阈值自动切换至讯飞并提示。
...
- 错误降级与自动切换
  - 当原生API出现网络错误且已配置讯飞凭证时，自动切换后端并短延时重试。
  - 若未配置讯飞凭证，则提示用户进行设置。
...
RetryCount --> |是| Switch{"已配置讯飞？"}
Switch --> |是| AutoSwitch["切换后端=XFYUN<br/>提示并延时重试"]
Switch --> |否| PromptCfg["提示配置讯飞凭证"]
PermErr --> End2["结束"]
NoSpeech --> End3["结束"]
Aborted --> End4["结束"]
AutoSwitch --> End5["结束"]
...
- 模块耦合
  ...
  - SpeechRecognition依赖XfyunSpeech与浏览器原生API。
  - XfyunSpeech依赖浏览器AudioContext与WebSocket。
- 外部依赖
  - Web Speech API（原生）与讯飞WebSocket服务。
  - localStorage用于配置持久化。
- 潜在风险
  - 原生API网络错误导致的降级路径需确保讯飞凭证有效。
...
SR --> XR["XfyunSpeech<br/>js/xfyun-speech.js"]
XR --> WS["WebSocket 讯飞"]
...
- 常见错误与定位
  ...
  - network：确认网络可达与原生服务可用性；若频繁出现，考虑切换至讯飞。
  - no-speech：确认麦克风设备可用与环境噪音控制。
  - aborted：观察是否被外部中断或资源释放。
- 降级与切换验证
  - 在原生网络错误后，确认是否自动切换至讯飞并提示。
...
- [xfyun-speech.js:298-310](file://js/xfyun-speech.js#L298-L310)
- [app.js:210-243](file://js/app.js#L210-L243)

## 结论
本项目通过清晰的状态机、完善的错误分类与降级策略、以及稳健的自动重连机制，实现了在复杂网络环境下可靠的语音识别体验。原生API与讯飞API的双后端设计，结合UI层面的即时反馈与持久化配置，使得系统具备良好的容错性与可维护性。建议在生产环境中进一步完善日志采集与错误上报，以便更精准地定位问题。
...
- 代码示例路径（不展示具体代码，仅提供定位）
  ...
  - 讯飞WebSocket连接与鉴权：[xfyun-speech.js:176-207](file://js/xfyun-speech.js#L176-L207)
  - 讯飞音频采集与帧发送：[xfyun-speech.js:94-102](file://js/xfyun-speech.js#L94-L102)
  - 讯飞错误上报与清理：[xfyun-speech.js:114-129](file://js/xfyun-speech.js#L114-L129)
