...
本指南专注于语音识别应用的错误处理和调试策略。该应用提供了两种语音识别后端：浏览器原生 Web Speech API 和讯飞语音识别服务，具备自动故障转移机制。本文档将详细介绍如何使用浏览器开发者工具进行调试，包括控制台日志分析、断点调试、网络请求监控等技巧，并提供常见错误代码和异常信息的处理方法。
...
### 讯飞语音客户端 (XfyunSpeech类)

XfyunSpeech类专门处理讯飞语音识别：
- WebSocket连接管理
- 音频数据捕获和传输
- 认证和签名处理
- 实时结果接收和解析
...
CheckRetry --> |是| AutoSwitch[自动切换到讯飞]
...
讯飞API的错误处理更加复杂，涉及多个层面：

```mermaid
flowchart TD
Start([讯飞启动失败]) --> CatchErr{捕获异常}
...
**图表来源**
- [speech.js:21-39](file://js/speech.js#L21-L39)
- [xfyun-speech.js:17-48](file://js/xfyun-speech.js#L17-L48)
- [app.js:12-41](file://js/app.js#L12-L41)
...
2. **关键日志位置**
   ...
   - 讯飞API错误: `console.error('讯飞语音识别启动失败:', ...)`
...
| `network` | 网络连接错误 | 自动切换到讯飞后端，提示配置API |
...
| `WebSocket连接失败` | 网络连接问题 | 检查网络设置和API配置 |
| `认证失败` | 凭证无效 | 提示重新配置API凭证 |
