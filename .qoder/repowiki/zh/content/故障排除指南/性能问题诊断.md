...
这是一个基于Web Speech API的语音识别应用，支持浏览器原生识别和讯飞语音识别两种后端。项目采用模块化设计，包含Canvas粒子背景动画、实时语音识别、多后端切换等功能。本文档专注于系统的性能问题诊断和优化策略，涵盖语音识别延迟、CPU占用过高、内存泄漏等问题的识别和解决方案。
...
CheckBackend --> |讯飞API| XfyunInit["初始化讯飞识别"]
NativeInit --> NativeMic["麦克风权限获取"]
NativeMic --> NativeStart["开始识别"]
NativeStart --> NativeResult["原生结果返回"]
XfyunInit --> XfyunMic["麦克风权限获取"]
XfyunMic --> XfyunAudio["创建AudioContext"]
XfyunAudio --> XfyunWS["建立WebSocket连接"]
XfyunWS --> XfyunResult["讯飞结果返回"]
NativeResult --> UpdateUI["更新UI"]
XfyunResult --> UpdateUI
UpdateUI --> End([结束])
```

**图表来源**
- [speech.js:154-184](file://js/speech.js#L154-L184)
- [xfyun-speech.js:77-108](file://js/xfyun-speech.js#L77-L108)
...
**章节来源**
- [xfyun-speech.js:384-391](file://js/xfyun-speech.js#L384-L391)
- [particles.js:152-167](file://js/particles.js#L152-L167)
- [app.js:182-208](file://js/app.js#L182-L208)
...
| 问题症状 | 可能原因 | 解决方案 |
|----------|----------|----------|
| 识别延迟明显 | 网络连接不稳定 | 切换到讯飞后端 |
| 原生API无响应 | 权限被拒绝 | 检查浏览器权限设置 |
| 讯飞API连接失败 | 网络环境限制 | 使用代理或VPN |
| 识别准确率低 | 音质问题 | 调整麦克风设置 |
